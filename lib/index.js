...
"use exports browser userAgent platformPackages platform console ready unload timer reactor";
...
var platformPackages = ['tiki/'+browser.current];
...
*/
var platform = function(moduleId) {
...
// export some common modules that paper over browser behavior
exports.console = require('logger').console;
exports.timer   = require('browser/timer'); // should not need this
exports.event = require('browser/event');
...
ready = function(target, method, args) {
  ...
  if (ready.isReady) {
    Invocation.invoke(target, method, arguments, offset);

  // if not ready yet, schedule in queue
  } else {
    if (!ready.scheduled) tiki.platform('event').ready(_ready);
    ready.scheduled = YES ;
    ready.queue.push(Invocation.create(target, method, arguments, offset));
  }
...
unload = function(target, method, args) {
  ...
  if (unload.isUnloaded) {
    ...
    unload.queue.push(Invocation.create(target, method, arguments, offset));
  }
