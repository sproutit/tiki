...
Browser.start = function(env, args, queue) {
  ...
  var source, len, idx, action;
  
  source         = new Browser();
  source.loader  = new Loader([source]);
  source.sandbox = new Sandbox(source.loader, env, args);
  source.queue   = queue;

  var mod = { 
    id: 'index', 
    ownerPackage: source.loader.anonymousPackage 
  };

  source.require = _createRequire(source.sandbox, mod);
