...
// _loadScript() actually loads the script.  separated out to ease unit testing
if (domAvailable) {
  if (exports.xhr && xhrAvailable) {
    Browser.prototype._loadScript = function(id, url) {
      try {
        this._xhrScript(id, url);
      } catch (e) {
        this._injectScript(id, url);
      }
    };
  } else {
    Browser.prototype._loadScript = Browser.prototype._injectScript;
  }
} else if (xhrAvailable) {
  Browser.prototype._loadScript = Browser.prototype._xhrScript;
} else {
  Browser.prototype._loadScript = function(id, url) {
