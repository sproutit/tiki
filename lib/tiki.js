...
NotFound.prototype.init = function(canonicalId, pkgId) {
  this.message = canonicalId+' not found in package '+pkgId;
  return this;
};
...
Package.prototype.ensurePackage = function(canonicalId, done) {
  ...
  else return done(new NotFound(canonicalId, this.id));
};
...
*/
Loader.prototype.load = function(canonicalId, workingPackage){
...
Loader.prototype._ensurePackage = function(packageId, vers, workingPackage, seen, done) {
  ...
  if (!canonicalId) {
    return done(new NotFound(packageId, workingPackage.id));
  }
  ...
  if (!source) {
    return done(new NotFound(canonicalId, workingPackage.id));
  }
...
Sandbox.prototype.module = function(moduleId, curModuleId, workingPackage) {
  ...
  if (!canonicalId) throw(new NotFound(moduleId, workingPackage.id));
  ...
  if (!pkg) throw(new NotFound(packageId, workingPackage.id));
...
Sandbox.prototype.require = function(moduleId, curModuleId, workingPackage) {
  ...
  factory = this.loader.load(canonicalId, workingPackage);
  if (!factory) throw(new NotFound(canonicalId, workingPackage.id));

  module  = this.module(canonicalId, workingPackage);
  cache[canonicalId] = module.exports;

  exp = factory.call(this, module);
  cache[canonicalId] = module.exports = exp;
...
Sandbox.prototype.ensure = function(moduleId, curModuleId, workingPackage, done) {
  ...
  if (!id) return done(new NotFound(moduleId, workingPackage.id));
  ...
  loader.ensurePackage(packageId, workingPackage, function(err) {
    ...
    if (!pkg.exists(moduleId)) done(new NotFound(moduleId, pkg.id));
    else done(); // all clear
  });
};
...
Sandbox.prototype.ensurePackage = function(packageId, vers, workingPackage, done) {
  ...
  var id = this.loader.canonicalPackageId(packageId, vers, workingPackage);
  if (!id) return done(new NotFound(packageId, workingPackage.id));
...
Browser.start = function(env, args, queue) {
  if (T_UNDEFINED === typeof window) {
    throw new Error("Cannot start Browser mode outside of browser");
  }

  // build new chain of objects and setup require.
  var tiki, len, idx, action;
  
  tiki         = new Browser();
  tiki.loader  = new Loader([tiki]);
  tiki.sandbox = new Sandbox(tiki.loader, env, args);

  var mod = { 
    id: 'index', 
    ownerPackage: tiki.loader.anonymousPackage 
  };
  tiki.require = _createRequire(tiki.sandbox, mod);
  // TODO: amend standard CommonJS methods for loading modules when they
  // are standardized

  window.tiki = tiki;
  if (!env.NO_REQUIRE) window.require = tiki.require;
  
  // now replay the queue if needed
  if (queue) {
    len = queue.length;
    for(idx=0;idx<len;idx++) {
      action = queue[idx];
      tiki.apply(action.method, action.args);
    }
    queue.length = 0; // release
  }
...
var queueListener = function(base, queueName, args) {
  ...
  base[queueName].push({ target: context, method: method, args: args });
};
...
/**
  ...
  @returns {void}
*/
Browser.prototype.main = function(moduleId) {
...
Browser.prototype._runMain = function() {
  var mainModuleId = this._main,
      req = this.require;
  if (!mainModuleId || !req) return;

  // async load any main module dependencies if needed then require
  req.ensure(mainModuleId, function(err) {
    if (err) throw err;
...
Browser.prototype._normalize = function(def, packageId) {
  ...
  if (def['tiki:resources']) {
    ...
    def['tiki:resources'] = map(def['tiki:resources'], function(item) {
      ...
      // must have an item id or else you can't lookup the resource
      if (!item.id) {
        throw new InvalidPackageDef(def, 'resources must have id');
      }
...
Browser.prototype.register = function(packageId, def) {
  ...
  if (reg[packageId]) {
    if (def.external || !reg[packageId].external) return this;
...
Browser.prototype.canonicalPackageId = function(packageId, vers) {
  ...
  // need to search...
  for(cvers in info) {
    if (!packages.hasOwnProperty(cvers)) continue;
...
Browser.prototype.ensurePackage = function(canonicalId, done) {
  ...
  action = once(function(done) {
    ...
    var resources = info['tiki:resources'], 
        loc = resources ? resources.length : 0,
        rsrc;
    while(--loc >= 0) {
      rsrc = resources[loc];
    ...
    // next, proactively kick off any known packages.  If a dependent package
    ...
    var dependencies = info.dependencies,
        ...
        packageId, vers, canonicalId, depInfo;

    for(packageId in dependencies) {
      if (!dependencies.hasOwnProperty(packageId)) continue;
      canonicalId = nested[packageId];
      if (!canonicalId) {
        vers = dependencies[packageId];
        canonicalId = source.canonicalPackageId(packageId, vers);
      }
      
      if (canonicalId && source.packageInfoById[canonicalId]) {
        cnt++;
        source.ensurePackage(canonicalId, cleanup);
      }
    }
...
Browser.prototype.ensureScript = function(id, url, done) {
  ...
  var source = this;
  action = this._action(function(done) {
    source._loadScript(id, url, done);
  });
...
Browser.prototype.ensureStylesheet = function(id, url, done) {
  var action = this.stylesheetAction[id];
  if (action) return action(done);
  ...
  this.stylesheetAction[id] = action;
  return action(done);
};
...
if ((T_UNDEFINED !== typeof document) && document.createElement) {
  ...
  Browser.prototype._loadScript = function(id, url, done) {
    var body, el;
  ...
  Browser.prototype._loadStylesheet = function(id, url, done) {
    var body, el;
...
BrowserPackage.prototype.canonicalPackageId = function(packageId, vers) {
  ...
  return info && (vers === info.version) ? ret : null;
};
