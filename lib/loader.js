...
// ==========================================================================
/*globals Loader sandbox promise setup create core */
...
Loader.prototype = {
  ...
  */
  ...
  /**
    ...
    This works similarly to resolve() except that passing an id with no 
    explicit package that matches package will actually return the package.
  ...
  */
  canonical: function canonical(moduleId, baseId) {
    var ret = this.resolve(moduleId, baseId),
        factories = this._factories,
        catalog   = this._catalog,
        packageId, idx;

    // if moduleId does not contain a packageId, then it MIGHT be a package
    if ((idx=moduleId.indexOf(':'))<0) {
      
      // named module exists...fast path
      if (!catalog || (factories && factories[ret])) return ret;

      // try mapping to a package
      if (catalog[moduleId]) return moduleId + ':package';
        
      // next, try to map to tiki package
      else if (catalog[inTiki(moduleId)]) return inTiki(moduleId)+':package';
...
setup = function setup(curLoader, env) {
  ...
  // bootstrap env
  if (!env) env = {};
  if (!env.global && ('undefined' !== typeof window)) env.global = window;
  if (!env.document && ('undefined' !== typeof document)) {
