...
// register a new source - goes in front of all the others
Loader.prototype.register = function(source, done) {
...
/**
  Converts some moduleId/packageId settings into a canonical id referencing
  the module itself
*/
Loader.prototype.canonical = function(moduleId, packageId, vers, module,done){
  var resolvedId;
  
  // already done 
  if (moduleId.indexOf('::')===0) {
    if (done) return done(null, moduleId);
    else return moduleId;
  }
  
  // normalize moduleId as much as possiblce
  resolvedId = this.resolve(moduleId, packageId, module.id, module.pkg);
  this.debug(moduleId, ' resolved to ', resolvedId);

  // TODO: finish porting back over
};

/** 
  Loads the passed module (and any of its dependents).
*/
Loader.prototype.load = function(moduleId, packageId, vers, module, done) {
  var id = this.canonical(moduleId, packageId, vers, module);
};
...
/**
  Finds the package for the named packageId/vers - can also pass optional
  context package used to determine compatible version for vers 
*/
Loader.prototype.packageFor = function(packageId, vers, pkg, done) {
  
};

Loader.prototype.packageList = function(pkg, done) {
  
};

Loader.prototype.requiredPackages = function(pkg, expand, done) {
  
};

Loader.prototype.canonicalPackageId = function(packageId, vers, pkg, done) {
