...
SC.Binding = {
  ...
  */
  fromPropertyDidChange: function(target, key) {
    ...
    //SC.logger.debug("fromPropertyDidChange: %@ v = %@".fmt(this, v)) ;
  ...
  */
  flushPendingChanges: function() {
    ...
    while ((queue = this._changeQueue).length > 0) {
      if (log) SC.logger.debug("Begin: Trigger changed bindings") ;
      ...
      // have triggered.
      if (log) SC.logger.debug("End: Trigger changed bindings") ;
    }
  ...
  */
  applyBindingValue: function() {
    ...
    if (!this._oneWay && this._fromTarget) {
      if (log) SC.logger.debug("%@: %@ -> %@".fmt(this, v, tv)) ;
