...
"import core";
// NOTE: other imports are in initObservable()
...
SC.Observable = {
  ...
  */
  propertyDidChange: function(key,value, _keepCache) {
    ...
    if ((level > 0) || suspended) {
      ...
      if (suspended) {
        if (log) SC.logger.debug("%@%@: will not notify observers because observing is suspended".fmt(SC.KVO_SPACES,this));
  ...
  */
  initObservable: function() {
    ...
    // we need to make sure these modules have loaded but importing them will
    // cause cyclical loops, so do it here once.
    if (!SC._observableRequired) {
      SC._observableRequired = YES;
  ...
  */
  ...
  _notifyPropertyObservers: function(key) {
    ...
    if (log) {
      ...
      SC.logger.debug('%@%@: notifying observers after change to key "%@"'.fmt(spaces, this, key));
    }
      ...
      if (dependents = this._kvo_dependents) {
        ...
        for(idx=0;idx<changes.length;idx++) {
          ...
          if (keys && (loc = keys.length)) {
            if (log) {
              SC.logger.debug("%@...including dependent keys for %@: %@".fmt(spaces, key, keys));
            }
            cache = this._kvo_cache;
            if (!cache) cache = this._kvo_cache = {};
            while(--loc >= 0) {
      ...
      while(changes.length > 0) {
        ...
        if (observers) {
          ...
          for(memberLoc=0;memberLoc < membersLength; memberLoc++) {
            ...
            if (log) SC.logger.debug('%@...firing observer on %@ for key "%@"'.fmt(spaces, target, key));
        ...
        if (members) {
          ...
          for(memberLoc=0;memberLoc<membersLength;memberLoc++) {
            ...
            if (method) {
              if (log) SC.logger.debug('%@...firing local observer %@.%@ for key "%@"'.fmt(spaces, this, member, key));
        ...
        if (this.propertyObserver) {
          if (log) SC.logger.debug('%@...firing %@.propertyObserver for key "%@"'.fmt(spaces, this, key));
  ...
  */
  logProperty: function() {
    ...
    for(var idx=0;idx<props.length; idx++) {
      var prop = props[idx] ;
      SC.logger.debug('%@:%@: '.fmt(SC.guidFor(this), prop), this.get(prop)) ;
    }
  },
...
SC.logChange = function logChange(target, key, value) {
  SC.logger.debug("CHANGE: %@[%@] => %@".fmt(target, key, target.get(key))) ;
};
